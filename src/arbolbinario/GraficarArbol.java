...
/**
 * Autor del algoritmo: Jarkko Leponiemi (http://people.uta.fi/~jl/) 
 * Tomado: http://people.uta.fi/~jl/pguibook/examples/BinaryTree_Java/BinaryTreeView.java
 *
 */
public class GraficarArbol<T> extends JPanel 
{
    private TADArbol<T> miArbol;
     ...
     * dibujar.
     */
    public GraficarArbol(TADArbol<T> miArbol) 
    {
    ...
    private void calcularPosiciones() 
    {
         posicionNodos.clear();
         subtreeSizes.clear();
         NodoArbol<T> root = this.miArbol.getRaiz();
         if (root != null) 
         {
     ...
     */
    private Dimension calcularTamañoSubarbol(NodoArbol<T> n) 
    {
          if (n == null) 
     ...
     */
    private void calcularPosicion(NodoArbol<T> n, int left, int right, int top) 
    {
      if (n == null) 
          return;
     ...
     */
    private void dibujarArbol(Graphics2D g, NodoArbol<T> n, int puntox, int puntoy, int yoffs) 
    {
     if (n == null) 
         return;
